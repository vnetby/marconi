...
class EdgeCoreImport {
	...
	public function set_import_url() {
		$params = EdgeCoreDashboard::get_instance()->get_import_params();
	...
	public function import_action() {
				...
				if ( wp_verify_nonce( $_POST['options']['nonce'], 'edgtf_cd_import_nonce' ) ) {
					...
					switch ($_POST['options']['action']):
						...
						case 'menu-settings':
							...
							break;
						case 'rev-slider':
							if (barista_edge_is_revolution_slider_installed()) {
								$this->rev_slider_import($demo);
							} else {
								$this->set_status('success');
								$this->set_data('type', 'options');
								$this->set_message(esc_html__('Revolution Slider isn\'t installed', 'edge-cpt'));
							}
							break;
						case 'content':
							...
							if($content_start) {
								if (!EdgeCoreDashboard::get_instance()->check_purchase_code($_POST['options']['demo'])) {
									edge_core_ajax_status('error', esc_html__('Please don\'t try to hack me. Purchase code registered is not valid', 'edge-cpt'));
									exit;
								}
							}
			...
			edge_core_ajax_status($this->get_status(), $this->get_message(), $this->get_data());
		}
		wp_die();
	}
	...
	function file_content( $path ) {
		...
		if ( '200' != wp_remote_retrieve_response_code( $response ) ) {
			$this->set_message($response["response"]['message'] . ' ' . esc_html__('Please contact support', 'edge-cpt'));
	...
	public function import_widgets($demo) {
		...
		if($widgets_content) {
			foreach ((array)$widgets_content['widgets'] as $barista_widget_id => $barista_widget_data) {
				update_option('widget_' . $barista_widget_id, $barista_widget_data);
			}
			$ws = $this->import_sidebars_widgets($widgets);
			if($ws) {
				$this->set_message(esc_html__('Widgets are set for proper sidebar', 'edge-cpt'));
				$this->set_data('type', 'options');
				$this->set_status('success');
			}
		}
	...
	public function import_sidebars_widgets( $file ) {
		$barista_sidebars = get_option( "sidebars_widgets" );
		unset( $barista_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$barista_sidebars = array_merge( (array) $barista_sidebars, (array) $data['sidebars'] );
			unset( $barista_sidebars['wp_inactive_widgets'] );
			$barista_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $barista_sidebars );
			$barista_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $barista_sidebars );
			return true;
		} else {
			return false;
		}
	...
	public function import_custom_sidebars( $file ) {
		...
		if($options) {
			$results = update_option('edgtf_sidebars', $options);
		...
		if($options){
			if($current_options != $options) {
				...
				if ($result) {
					...
					$this->set_status('success');
					$this->set_data('type', 'options');
					$this->set_message(esc_html__('Options imported successfully', 'edge-cpt'));

					$this->update_options_after_import($file);

				} else {
					$this->set_status('error');
					$this->set_message(esc_html__('Problem occurred during options import', 'edge-cpt'));
				}
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Options are already imported', 'edge-cpt'));
			}
		}
	...
	public function import_settings_pages( $file ) {
		...
		$new_ids = get_transient( '_barista_core_imported_posts' );
		$fields_status = true;
		...
		if($pages) {
			...
			if (!$fields_status) {
				$this->set_status('error');
				$this->set_message(esc_html__('Problem occurred during settings pages import', 'edge-cpt'));
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Settings pages imported successfully', 'edge-cpt'));
			}
		} else {
			$this->set_status('error');
			$this->set_message(esc_html__('File doesn\'t exist', 'edge-cpt'));
		}
	}
	...
	public function import_menu_settings( $file ) {
		...
		if($menus_data) {
			...
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message( esc_html__( 'Menus set for proper locations', 'edge-cpt' ) );
		} else {
			$this->set_status('error');
			$this->set_message( esc_html__( 'Problem during menus location set', 'edge-cpt' ) );
		}
	}
...
//
		...
		$barista_import = new WP_Import();
		set_time_limit( 0 );

		$barista_import->fetch_attachments = $attachments;
		$returned_value                  = $barista_import->import( $file . $xml );
		...
		if ( is_wp_error( $returned_value ) ) {
			...
			$this->set_message( esc_html__( 'An error occurred during content import', 'edge-cpt' ) );
		} else {
			$this->set_status('success');
			$this->set_data('type', 'content');
			$this->set_data('posts', $this->imported_posts);
			$this->set_message( esc_html__( 'File imported successfully', 'edge-cpt' ) . ' ' . $xml );
		...
		if($files_created) {
			...
			foreach ($rev_sliders as $rev_slider) {
				...
				if (!$rev_results['success']) {
					$this->set_status('error');
					$this->set_message(esc_html__('Error while importing rev sliders', 'edge-cpt'));
					exit;
				}
			}
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Rev sliders imported successfully', 'edge-cpt'));
		} else {
			$this->set_status('error');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Files don\'t exist', 'edge-cpt'));
		}
	}
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && !empty( $_POST ) && !empty($_POST['options']['demo']) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'edgtf_cd_import_nonce' ) ) {
				...
				$html = edgt_core_get_module_template_part('core-dashboard/sub-pages/import', 'pages-list', '', array('pages' => $pages));

				if($pages){
					edge_core_ajax_status( 'success', '', $html);
				} else {
					edge_core_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'edge-cpt' ), '');
				}
			}
		}
	...
	public function is_ready_to_import() {
		$info = EdgeCoreSystemInfoPage::get_instance()->get_system_info();
