...
class EdgefInstagramApi {
    ...
    public function getAccessToken() {
        ...
        if(empty($this->code)) {
            $returnObject->status = 'error';
            $returnObject->message = esc_html__('User has\'nt connected with Instagram','edge-instagram-feed');
            ...
            if(is_wp_error($httpResponse) || (isset($httpResponse['response']['code']) && $httpResponse['response']['code'] !== 200)) {
                $returnObject->status = 'error';
                $returnObject->message = esc_html__('Can\t connect with Instagram API','edge-instagram-feed');
            ...
            if(isset($responseBody['access_token']) && isset($responseBody['user']['id'])) {
                ...
                $returnObject->status = 'error';
                $returnObject->message = esc_html__('Can\t get Instagram access token','edge-instagram-feed');
