...
if(!function_exists('barista_edge_single_portfolio')) {
    function barista_edge_single_portfolio() {
        $portfolio_template = barista_edge_get_portfolio_single_type();
        ...
        if ($portfolio_template == 'gallery') {
            $params['holder_class'][] = 'edgtf-portfolio-gallery-' . barista_edge_options()->getOptionValue('portfolio_single_numb_columns');
        }

        barista_edge_get_module_template_part('templates/single/holder', 'portfolio', '', $params);
    }
}

if(!function_exists('barista_edge_get_portfolio_single_type')) {
    function barista_edge_get_portfolio_single_type() {
        return barista_edge_get_meta_field_intersect('portfolio_single_template');
    }
}

if(!function_exists('barista_edge_get_portfolio_single_media')) {
    function barista_edge_get_portfolio_single_media() {
	    ...
	    $single_type = barista_edge_get_portfolio_single_type();
        if($image_ids !== '') {
        ...
        if(is_array($videos) && count($videos)) {
            usort($videos, 'barista_edge_compare_portfolio_videos');
            foreach($videos as $video) {
                ...
                if(!empty($video['portfoliovideotype'])) {
                    ...
                    $media['video_url']   = barista_edge_portfolio_get_video_url($video);
...
if(!function_exists('barista_edge_portfolio_get_video_url')) {
    function barista_edge_portfolio_get_video_url($video) {
...
if(!function_exists('barista_edge_portfolio_get_media_html')) {
    function barista_edge_portfolio_get_media_html($media) {
        global $wp_filesystem;
        $params = array();

        //For adding image overlay in gallery template type
        $params['gallery'] = (barista_edge_get_portfolio_single_type() == 'gallery') ? true : false;

        if($media['type'] == 'image') {
            $params['lightbox'] = barista_edge_options()->getOptionValue('portfolio_single_lightbox_images') == 'yes';
        ...
        if(in_array($media['type'], array('youtube', 'vimeo'))) {
            $params['lightbox'] = barista_edge_options()->getOptionValue('portfolio_single_lightbox_videos') == 'yes';
        ...
        barista_edge_get_module_template_part('templates/single/media/'.$media['type'], 'portfolio', '', $params);
    }
}

if(!function_exists('barista_edge_compare_portfolio_videos')) {
    /**
     ...
     */
    function barista_edge_compare_portfolio_videos($a, $b) {
...
if(!function_exists('barista_edge_compare_portfolio_options')) {
    /**
     ...
     * @return int result of comparison
     */
    function barista_edge_compare_portfolio_options($a, $b) {
...
if(!function_exists('barista_edge_portfolio_get_info_part')) {
    function barista_edge_portfolio_get_info_part($part) {
        $portfolio_template = barista_edge_get_portfolio_single_type();

        barista_edge_get_module_template_part('templates/single/parts/'.$part, 'portfolio', $portfolio_template, array('type'=>$portfolio_template));
    }
}