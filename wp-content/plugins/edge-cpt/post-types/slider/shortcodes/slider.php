...
class Slider implements Lib\ShortcodeInterface {
	...
	public function render( $atts, $content = null ) {
		...
		if ( $slider != "" ) {
			...
			if ( $barista_edge_options['smooth_page_transitions'] == "yes" && $barista_edge_options['smooth_pt_spinner_type'] != "" ) {
				$ajax_loader = '<div class="edgtf-st-loader"><div class="edgtf-st-loader1">' . barista_edge_loading_spinners( true ) . '</div></div>';
			} else {
				$ajax_loader = '<div class="edgtf-st-loader"><div class="edgtf-st-loader1">' . barista_edge_loading_spinner_pulse() . '</div></div>';
			}

			/* set slider preloader - end */

			/* set padding for slider arrows - start */
			$slider_arrows_padding = "padding-top: " . esc_attr( $this->barista_edge_get_slider_navigation_padding() ) . "px;";
			/* set padding for slider arrows - end */

			$html .= '<div id="edgtf-' . esc_attr( $slider ) . '" ' . $slider_anchor . ' class="carousel slide ' . esc_attr( $animation_type_class . ' ' . $full_screen_class . ' ' . $responsive_height_class . ' ' . $height_class . ' ' . $auto_start_class . ' ' . $header_effect_class . ' ' . $slider_numbers_class . ' ' . $slider_thumbs_class ) . ' " ' . $slide_animation_timeout . ' ' . $data_height . ' ' . $data_parallax_effect . ' ' . barista_edge_get_inline_style( $slide_holder_height ) . '><div class="edgtf-slider-preloader">' . $ajax_loader . '</div>';
			$html .= '<div class="carousel-inner ' . esc_attr( $slider_css_position_class ) . '" ' . barista_edge_get_inline_style( $carouselinner_height ) . ' ' . $data_parallax_transform . '>';
			...
			if ( have_posts() ) : $postCount = 0;
				while ( have_posts() ) : the_post();
					...
					if ( $slide_type == 'video' ) {
						...
						$html .= '<div class="edgtf-video"><div class="edgtf-mobile-video-image" ' . barista_edge_get_inline_style( 'background-image: url(' . esc_url( $video_image ) . ')' ) . '></div><div class="edgtf-video-overlay';
						...
						$html .= '>';
						if ( $video_overlay_image != "" ) {
							$html .= '<img src="' . esc_url( $video_overlay_image ) . '" alt="' . esc_attr__( 'Video overlay image', 'edge-cpt' ) . '" />';
						} else {
							$html .= '<img src="' . esc_url( get_template_directory_uri() ) . '/assets/css/img/pixel-video.png" alt="' . esc_attr( 'Pixel video', 'edge-cpt' ) . '" />';
						}
						...
						$html .= '<div class="edgtf-image" ' . barista_edge_get_inline_style( 'background-image:url(' . esc_url( $image ) . ')' ) . '>';
						if ( $slider_thumbs == 'no' ) {
							$html .= '<img src="' . esc_url( $image ) . '" alt="' . esc_attr( $title ) . '">';
						}

						if ( $image_overlay_pattern !== "" ) {
							$html .= '<div class="edgtf-image-pattern" ' . barista_edge_get_inline_style( 'background: url(' . esc_url( $image_overlay_pattern ) . ') repeat 0 0' ) . '></div>';
						}
						$html .= '</div>';
					}
								...
								case 'text':
									...
									if ( $text_options == 'advanced' ) {
										...
										if ( $slide_element['slideelementtextbgndcolor'] != '' ) {
											...
											$style_string .= 'background-color: ' . barista_edge_rgba_color( $slide_element_bg_color, $slide_element_bg_transparency ) . ';';
										}
								...
								case 'button':
									...
									if ( $slide_element['slideelementbuttoniconpack'] != '' ) {
										...
										if ( $button_params['icon_pack'] != 'no_icon' ) {
											$iconPackName = barista_edge_icon_collections()->getIconCollectionParamNameByKey( $button_params['icon_pack'] );
									...
									$slide_element_html .= '<div class="' . $class_string . '" ' . $data_string . ' style="' . $style_string . '"><div class="edgtf-slide-element-inner" style="' . $inner_style_string . '">' . barista_edge_get_button_html( $button_params ) . '</div></div>';
									break;
				...
				if ( $show_navigation_circles == "yes" ) {
					...
					if ( have_posts() ) : $postCount = 0;
						while ( have_posts() ) : the_post();
							...
							$slide_type = get_post_meta( get_the_ID(), "edgtf_slide_background_type", true );
							if ( $slide_type == 'image' ) {
								$img .= barista_edge_generate_thumbnail( '', esc_url( get_post_meta( get_the_ID(), "edgtf_slide_image", true ) ), '160', '100' );
							} elseif ( $slide_type == 'video' ) {
								$img .= barista_edge_generate_thumbnail( '', esc_url( get_post_meta( get_the_ID(), "edgtf_slide_video_image", true ) ), '160', '100' );
							}
							$html .= '>';
							$html .= $img;
							$html .= '</li>';
	 ...
	 **/
	function barista_edge_get_slider_item_padding() {
		return apply_filters( 'barista_edge_slider_item_padding', 0 );
	}

	/**
	 * Function that returns slider navigation pading
	 **/
	function barista_edge_get_slider_navigation_padding() {
		return apply_filters( 'barista_edge_slider_navigation_padding', 0 );
	}
}