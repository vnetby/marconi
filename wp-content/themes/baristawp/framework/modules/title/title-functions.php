...
if(!function_exists('barista_edge_get_title')) {
    /**
     * Loads title area HTML
     */
    function barista_edge_get_title() {
        $id = barista_edge_get_page_id();

        extract(barista_edge_title_area_height());
        extract(barista_edge_title_area_background());
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_show_title_area_meta", true)) !== ""){
            ...
            $show_title_area = barista_edge_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
        }
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_type_meta", true)) !== ""){
            ...
            $type = barista_edge_options()->getOptionValue('title_area_type');
        }
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_enable_breadcrumbs_meta", true)) !== ""){
            ...
            $enable_breadcrumbs = barista_edge_options()->getOptionValue('title_area_enable_breadcrumbs') == 'yes' ? true : false;
        }

		//check if separator are enabled on page first, then in options
		$enable_separator_temp = barista_edge_get_meta_field_intersect('title_area_enable_separator', $id);
		$enable_separator = $enable_separator_temp == 'yes' ? true : false;
		//seaprator options
		$separator_args = array();
		if($enable_separator) {

			$separator_args['position'] = barista_edge_get_meta_field_intersect('title_area_content_alignment', $id);
        ...
        $parameters = apply_filters('barista_edge_title_area_height_params', $parameters);

        barista_edge_get_module_template_part('templates/title', 'title', '', $parameters);
    }
}

if(!function_exists('barista_edge_get_title_text')) {
    /**
     * Function that returns current page title text. Defines barista_edge_title_text filter
     ...
     */
    function barista_edge_get_title_text() {
        ...
        if (is_tag()) {
            ...
            $title = single_term_title("", false).esc_html__(' Tag', 'baristawp');
        }
        ...
        elseif (is_author()) {
            ...
            $title = esc_html__('Author:', 'baristawp') . " " . get_the_author();
        }
        ...
        elseif (is_singular('post')) {
            if (barista_edge_options()->getOptionValue('blog_single_title_in_title_area') == "yes") {
        ...
        elseif (is_404()) {
            ...
            if(barista_edge_options()->getOptionValue('404_title') != "") {
                //get it from options
                $title = barista_edge_options()->getOptionValue('404_title');
            } else {
                //get default 404 page title
                $title = esc_html__('404 - Page not found', 'baristawp');
            }
        }

        //is WooCommerce installed and is shop or single product page?
        elseif(barista_edge_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
        ...
        elseif(barista_edge_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
            global $wp_query;
        ...
        elseif (is_archive()) {
            $title = esc_html__('Archive','baristawp');
        }
        ...
        $title = apply_filters('barista_edge_title_text', $title);

        return $title;
    }
}

if(!function_exists('barista_edge_title_text')) {
    /**
     * Function that echoes title text.
     *
     * @see barista_edge_get_title_text()
     */
    function barista_edge_title_text() {
        echo barista_edge_get_title_text();
    }
}

if(!function_exists('barista_edge_subtitle_text')) {
    /**
     * Function that echoes subtitle text.
     *
     */
    function barista_edge_subtitle_text() {
        $id = barista_edge_get_page_id();
        $subtitle 	= '';
        ...
        print barista_edge_get_module_part($subtitle);
    }
}

if(!function_exists('barista_edge_custom_breadcrumbs')) {
    /**
     ...
     * @see is_front_page()
     * @see is_category()
     * @see barista_edge_is_product_category()
     * @see get_search_query()
     */
    function barista_edge_custom_breadcrumbs() {
        global $post, $wp_query;

        $output = "";
        $homeLink = esc_url(home_url('/'));
        $pageid = barista_edge_get_page_id();
        $bread_style = "";
        ...
        $delimiter = '<span class="edgtf-delimiter" '.barista_edge_get_inline_style($bread_style).'>&nbsp;/&nbsp;</span>'; // delimiter between crumbs
        $home = get_bloginfo('name'); // text for the 'Home' link
        $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
        $before = '<span class="edgtf-current" '.barista_edge_get_inline_style($bread_style).'>'; // tag before the current crumb
        $after = '</span>'; // tag after the current crumb

        if (is_home() && !is_front_page()) {
            $output = '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner"><a itemprop="url" '.barista_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a '.barista_edge_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';
        ...
        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner"><a itemprop="url" '.barista_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></div></div>';
        }

        else {

            $output .= '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner"><a itemprop="url" '.barista_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;

            if ( is_category() || barista_edge_is_product_category()) {
                ...
                $output .= $before . esc_html__('Search results for ', 'baristawp') . '"' . get_search_query() . '"' . $after;

            } elseif ( is_day() ) {
                $output .= '<a itemprop="url" '.barista_edge_get_inline_style($bread_style).' href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a>' . $delimiter;
                $output .= '<a itemprop="url" '.barista_edge_get_inline_style($bread_style).' href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a>' . $delimiter;
                ...
                $output .= '<a itemprop="url" '.barista_edge_get_inline_style($bread_style).' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a>';
                ...
                while ($parent_id) {
                    ...
                    $breadcrumbs[] = '<a itemprop="url" '.barista_edge_get_inline_style($bread_style).' href="' . get_permalink($page->ID) . '">' . get_the_title($page->ID) . '</a>';
                ...
                $output .= $before . esc_html__('Posts tagged ','baristawp') .'"' . single_tag_title('', false) . '"' . $after;

            } elseif ( is_author() ) {
                global $authordata;
                $output .= $before . esc_html__('Articles posted by ','baristawp') . $authordata->display_name . $after;
            ...
            if ( get_query_var('paged') ) {
                ...
                $output .= $before . " (" . esc_html__('Page', 'baristawp') . ' ' . get_query_var('paged') . ")" . $after;
...
if(!function_exists('barista_edge_get_title_area_height')) {

    /**
     * Function that returns title height
     **/
    function barista_edge_get_title_area_height() {
        ...
        } elseif(barista_edge_options()->getOptionValue('title_area_height') !== '') {
            $title_height = barista_edge_options()->getOptionValue('title_area_height');
        }

        return apply_filters('barista_edge_title_area_height', $title_height);
    }
}

if(!function_exists('barista_edge_get_title_content_padding')) {
    /**
     * Function that returns title content pading
     **/

    function barista_edge_get_title_content_padding() {
        return apply_filters('barista_edge_title_content_padding', 0);
    }
}


if(!function_exists('barista_edge_title_area_height')) {
    /**
     * Function that returns title height and padding to be applied in template
     **/

    function barista_edge_title_area_height() {
        $id = barista_edge_get_page_id();
        $title_height_and_padding = array();
        $title_height          = barista_edge_get_title_area_height();
        $header_height_padding = barista_edge_get_title_content_padding();
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_vertial_alignment_meta", true)) !== '') {
            ...
            $title_vertical_alignment = barista_edge_options()->getOptionValue('title_area_vertial_alignment');;
        }
...
if(!function_exists('barista_edge_title_area_background')) {
    /**
     * Function that returns title background style be applied in template
     **/

    function barista_edge_title_area_background() {
        $id = barista_edge_get_page_id();
        $show_title_img = true;
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_background_color_meta", true)) != ""){
            ...
            $background_color = barista_edge_options()->getOptionValue('title_area_background_color');
        }
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_background_image_meta", true)) != ""){
            ...
            $background_image = barista_edge_options()->getOptionValue('title_area_background_image');
        }

        //check for background image width
        $background_image_width = "";
        if($background_image !== ''){
            $background_image_width_dimensions_array = barista_edge_get_image_dimensions($background_image);
