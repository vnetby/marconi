 ...
 * It first checks global $barista_edge_options variable and if option does'nt exists there
 ...
 * @return bool
 */
function barista_edge_option_has_value( $name ) {
	global $barista_edge_options;
	global $barista_edge_Framework;

	if ( array_key_exists( $name, $barista_edge_Framework->edgtOptions->options ) ) {
		if ( isset( $barista_edge_options[ $name ] ) ) {
			return true;
		} else {
			return false;
		}
 ...
 * It first checks if option exists in $barista_edge_options global array and if it does'nt exists there
 ...
 * @return mixed|void
 */
function barista_edge_option_get_value( $name ) {
	global $barista_edge_options;
	global $barista_edge_Framework;

	if ( array_key_exists( $name, $barista_edge_Framework->edgtOptions->options ) ) {
		if ( isset( $barista_edge_options[ $name ] ) ) {
			return $barista_edge_options[ $name ];
		} else {
			return $barista_edge_Framework->edgtOptions->getOption( $name );
		}
	} else {
		global $post;
		...
		if ( isset( $value ) && $value !== "" ) {
			...
			return $barista_edge_Framework->edgtMetaBoxes->getOption( $name );
		}
 ...
 * @return bool|string
 *
 * @see barista_edge_get_attachment_id_from_url()
 */
function barista_edge_get_attachment_thumb_url( $attachment_url ) {
	$attachment_id = barista_edge_get_attachment_id_from_url( $attachment_url );
 ...
 */
function barista_edge_enqueue_skin_style( $handle, $src, $deps = array(), $ver = false, $media = 'all' ) {
	global $barista_edge_Framework;

	$src = get_template_directory_uri() . '/framework/admin/skins/' . $barista_edge_Framework->getSkin() . '/' . $src;
 ...
 */
function barista_edge_register_skin_style( $handle, $src, $deps = array(), $ver = false, $media = 'all' ) {
	global $barista_edge_Framework;
 ...
 */
function barista_edge_enqueue_skin_script( $handle, $src, $deps = array(), $ver = false, $in_footer = false ) {
	global $barista_edge_Framework;
 ...
 */
function barista_edge_register_skin_script( $handle, $src, $deps = array(), $ver = false, $in_footer = false ) {
	global $barista_edge_Framework;
...
if ( ! function_exists( 'barista_edge_generate_dynamic_css_and_js' ) ) {
	/**
	 * Function that gets content of dynamic assets files and puts that in static ones
	 */
	function barista_edge_generate_dynamic_css_and_js() {
		global $wp_filesystem;
		WP_Filesystem();

		if ( barista_edge_is_css_folder_writable() ) {
...
if ( ! function_exists( 'barista_edge_gallery_upload_get_images' ) ) {
	 ...
	 */
	function barista_edge_gallery_upload_get_images() {
	...
	add_action( 'wp_ajax_barista_edge_gallery_upload_get_images', 'barista_edge_gallery_upload_get_images' );
}

if ( ! function_exists( 'barista_edge_hex2rgb' ) ) {
	/**
	 ...
	 */
	function barista_edge_hex2rgb( $hex ) {
		$hex = str_replace( "#", "", $hex );
...
if ( ! function_exists( 'barista_edge_addslashes' ) ) {
	/**
	 ...
	 */
	function barista_edge_addslashes( $str ) {
...
if ( ! function_exists( 'barista_edge_get_attachment_meta' ) ) {
	/**
	 ...
	 * @return array|mixed
	 */
	function barista_edge_get_attachment_meta( $attachment_id, $keys = array() ) {
		$meta_data = array();
...
if ( ! function_exists( 'barista_edge_get_attachment_id_from_url' ) ) {
	/**
	 ...
	 * @return null|string
	 */
	function barista_edge_get_attachment_id_from_url( $attachment_url ) {
		global $wpdb;
		$attachment_id = '';
...
if ( ! function_exists( 'barista_edge_get_attachment_meta_from_url' ) ) {
	/**
	 ...
	 *
	 * @see barista_edge_get_attachment_id_from_url()
	 * @see barista_edge_get_attachment_meta()
	 *
	 * @version 0.1
	 */
	function barista_edge_get_attachment_meta_from_url( $attachment_url, $keys = array() ) {
		...
		if ( ! empty( $attachment_id ) ) {
			//get post meta
			$attachment_meta = barista_edge_get_attachment_meta( $attachment_id, $keys );
		}
...
if ( ! function_exists( 'barista_edge_get_image_dimensions' ) ) {
	/**
	 ...
	 *
	 * @see  barista_edge_get_attachment_meta_from_url()
	 * @uses getimagesize
	 *
	 * @version 0.1
	 */
	function barista_edge_get_image_dimensions( $url ) {
		$image_sizes = array();

		//is url passed?
		if ( $url !== '' ) {
			//get image sizes from posts meta if attachment exists
			$image_sizes = barista_edge_get_attachment_meta_from_url( $url, array( 'width', 'height' ) );
...
if ( ! function_exists( 'barista_edge_get_native_fonts_list' ) ) {
	/**
	 * Function that returns array of native fonts
	 * @return array
	 */
	function barista_edge_get_native_fonts_list() {
...
if ( ! function_exists( 'barista_edge_get_native_fonts_array' ) ) {
	/**
	 * Function that returns formatted array of native fonts
	 *
	 * @return array
	 * @uses barista_edge_get_native_fonts_list()
	 */
	function barista_edge_get_native_fonts_array() {
		$native_fonts_list  = barista_edge_get_native_fonts_list();
		$native_font_index  = 0;
...
if ( ! function_exists( 'barista_edge_is_native_font' ) ) {
	/**
	 ...
	 * @return bool
	 */
	function barista_edge_is_native_font( $font_family ) {
		return in_array( str_replace( '+', ' ', $font_family ), barista_edge_get_native_fonts_list() );
	}
}


if ( ! function_exists( 'barista_edge_merge_fonts' ) ) {
	/**
	 * Function that merge google and native fonts
	 *
	 * @return array
	 * @uses barista_edge_get_native_fonts_array()
	 */
	function barista_edge_merge_fonts() {
		global $barista_edge_fonts_array;
		$native_fonts = barista_edge_get_native_fonts_array();
		if ( ! barista_edge_core_installed() ) {
			return;
		}
		if ( is_array( $native_fonts ) && count( $native_fonts ) ) {
			$barista_edge_fonts_array = array_merge( $native_fonts, $barista_edge_fonts_array );
		}
	}

	add_action( 'admin_init', 'barista_edge_merge_fonts' );
}

if ( ! function_exists( 'barista_edge_is_css_folder_writable' ) ) {
	/**
	 ...
	 * @uses is_writable()
	 */
	function barista_edge_is_css_folder_writable() {
		$css_dir = EDGE_ASSETS_ROOT_DIR . '/css';
...
if ( ! function_exists( 'barista_edge_is_js_folder_writable' ) ) {
	/**
	 ...
	 * @uses is_writable()
	 */
	function barista_edge_is_js_folder_writable() {
...
if ( ! function_exists( 'barista_edge_assets_folders_writable' ) ) {
	/**
	 * Function that if css and js folders are writable
	 * @return bool
	 *
	 * @version 0.1
	 * @see barista_edge_is_css_folder_writable()
	 * @see barista_edge_is_js_folder_writable()
	 */
	function barista_edge_assets_folders_writable() {
...
if ( ! function_exists( 'barista_edge_writable_assets_folders_notice' ) ) {
	 ...
	 */
	function barista_edge_writable_assets_folders_notice() {
		global $pagenow;

		$is_theme_options_page = isset( $_GET['page'] ) && strstr( $_GET['page'], 'barista_edge_theme_menu' );
		...
		if ( $pagenow === 'admin.php' && $is_theme_options_page ) {
			if ( ! barista_edge_assets_folders_writable() ) { ?>
                <div class="error">
					...
					For further instructions, please refer to our documentation.', 'baristawp' ); ?></p>
                </div>
			<?php }
		}
	}

	add_action( 'admin_notices', 'barista_edge_writable_assets_folders_notice' );
}

if ( ! function_exists( 'barista_edge_inline_style' ) ) {
	/**
	 * Function that echoes generated style attribute
	 *
	 * @param $value string | array attribute value
	 *
	 * @see barista_edge_get_inline_style()
	 */
	function barista_edge_inline_style( $value ) {
		echo barista_edge_get_inline_style( $value );
	}
}

if ( ! function_exists( 'barista_edge_get_inline_style' ) ) {
	/**
	 ...
	 * @see barista_edge_get_inline_style()
	 */
	function barista_edge_get_inline_style( $value ) {
		return barista_edge_get_inline_attr( $value, 'style', ';' );
	}
}

if ( ! function_exists( 'barista_edge_class_attribute' ) ) {
	/**
	 * Function that echoes class attribute
	 *
	 * @param $value string value of class attribute
	 *
	 * @see barista_edge_get_class_attribute()
	 */
	function barista_edge_class_attribute( $value ) {
		echo barista_edge_get_class_attribute( $value );
	}
}

if ( ! function_exists( 'barista_edge_get_class_attribute' ) ) {
	/**
	 ...
	 *
	 * @see barista_edge_get_inline_attr()
	 */
	function barista_edge_get_class_attribute( $value ) {
		return barista_edge_get_inline_attr( $value, 'class', ' ' );
	}
}

if ( ! function_exists( 'barista_edge_get_inline_attr' ) ) {
	/**
	 ...
	 */
	function barista_edge_get_inline_attr( $value, $attr, $glue = '' ) {
		if ( ! empty( $value ) ) {
...
if ( ! function_exists( 'barista_edge_inline_attr' ) ) {
	/**
	 * Function that generates html attribute
	 *
	 * @param $value string | array value of html attribute
	 ...
	 * @return string generated html attribute
	 */
	function barista_edge_inline_attr( $value, $attr, $glue = '' ) {
		echo barista_edge_get_inline_attr( $value, $attr, $glue );
	}
}

if ( ! function_exists( 'barista_edge_get_inline_attrs' ) ) {
	/**
	 ...
	 * @return string
	 */
	function barista_edge_get_inline_attrs( $attrs ) {
		$output = '';

		if ( is_array( $attrs ) && count( $attrs ) ) {
			foreach ( $attrs as $attr => $value ) {
				$output .= ' ' . barista_edge_get_inline_attr( $value, $attr );
			}
		}
...
if ( ! function_exists( 'barista_edge_get_skin_uri' ) ) {
	/**
	 * Returns current skin URI
	 * @return mixed
	 */
	function barista_edge_get_skin_uri() {
		global $barista_edge_Framework;

		$current_skin = $barista_edge_Framework->getSkin();

		return $current_skin->getSkinURI();
	}
}

if ( ! function_exists( 'barista_edge_core_installed' ) ) {
	/**
	 * Checks if core plugin is installed
	 * @return bool
	 */
	function barista_edge_core_installed() {
		return defined( 'EDGE_CORE_VERSION' );
	}
}

if ( ! function_exists( 'barista_edge_core_plugin_message' ) ) {
	/**
	 * Function that prints a mesasge in the admin if user hides TGMPA plugin activation message
	 */
	function barista_edge_core_plugin_message() {
		if ( get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) && ! barista_edge_core_installed() ) {
			echo apply_filters( 'barista_edge_core_plugin_message', '<div class="update-nag">' . esc_html__( 'Installation of the "Edge
            Core" plugin is
            essential for
            proper
            theme functioning. Please ', 'baristawp' ) . '<a href="' . esc_url( admin_url( 'themes.php?page=install-required-plugins' ) )
			                                                        . '">' . esc_html__( 'install', 'baristawp' ) . '</a>' . esc_html__( ' this plugin and activate it', 'baristawp' ) . '</div>' );
		}
	}

	add_action( 'admin_notices', 'barista_edge_core_plugin_message' );
}

if ( ! function_exists( 'barista_edge_get_theme_info_item' ) ) {
	/**
	 ...
	 * @return string
	 */
	function barista_edge_get_theme_info_item( $item ) {
		if ( $item !== '' ) {
...
if ( ! function_exists( 'barista_edge_resize_image' ) ) {
	/**
	 ...
	 */
	function barista_edge_resize_image( $attach_id = null, $attach_url = null, $width = null, $height = null, $crop = true ) {
		$return_array = array();

		//is attachment id empty?
		if ( empty( $attach_id ) && $attach_url !== '' ) {
			//get attachment id from url
			$attach_id = barista_edge_get_attachment_id_from_url( $attach_url );
		}
...
if ( ! function_exists( 'barista_edge_generate_thumbnail' ) ) {
	/**
	 * Generates thumbnail img tag. It calls barista_edge_resize_image function which resizes img on the fly
	 *
	 ...
	 *
	 * @see barista_edge_resize_image()
	 * @see barista_edge_get_attachment_id_from_url()
	 */
	function barista_edge_generate_thumbnail( $attach_id = null, $attach_url = null, $width = null, $height = null, $crop = true ) {
		//is attachment id empty?
		if ( empty( $attach_id ) ) {
		...
		if ( ! empty( $attach_id ) || ! empty( $attach_url ) ) {
			$img_info = barista_edge_resize_image( $attach_id, $attach_url, $width, $height, $crop );
...
if ( ! function_exists( 'barista_edge_get_template_part' ) ) {
	/**
	 ...
	 */
	function barista_edge_get_template_part( $template, $slug = '', $params = array() ) {
		...
		$located = barista_edge_find_template_path( $templates );
...
if ( ! function_exists( 'barista_edge_get_module_template_part' ) ) {
	/**
	 ...
	 * @param array $params array of parameters to pass to template
	 *
	 * @see barista_edge_get_template_part()
	 */
	function barista_edge_get_module_template_part( $template, $module, $slug = '', $params = array() ) {
		$template_path = 'framework/modules/' . $module;

		barista_edge_get_template_part( $template_path . '/' . $template, $slug, $params );
	}
}


if ( ! function_exists( 'barista_edge_get_shortcode_module_template_part' ) ) {
	/**
	 * Loads module template part.
	 *
	 * @param string $template name of the template to load
	 ...
	 * @see barista_edge_get_template_part()
	 */
	function barista_edge_get_shortcode_module_template_part( $template, $module, $slug = '', $params = array() ) {
...
if ( ! function_exists( 'barista_edge_get_blog_module_template_part' ) ) {
	/**
	 * Loads module template part.
	 *
	 * @param string $template name of the template to load
	 ...
	 * @see barista_edge_get_template_part()
	 */
	function barista_edge_get_blog_module_template_part( $module, $slug = '', $params = array() ) {
...
if ( ! function_exists( 'barista_edge_find_template_path' ) ) {
	/**
	 ...
	 * @return string
	 */
	function barista_edge_find_template_path( $template_names ) {
		$located = '';
...
if ( ! function_exists( 'barista_edge_filter_suffix' ) ) {
	/**
	 ...
	 * @return string
	 */
	function barista_edge_filter_suffix( $value, $suffix ) {
		if ( $value !== '' && barista_edge_string_ends_with( $value, $suffix ) ) {
...
if ( ! function_exists( 'barista_edge_filter_px' ) ) {
	/**
	 ...
	 * @return string
	 *
	 * @see barista_edge_filter_suffix
	 */
	function barista_edge_filter_px( $value ) {
		return barista_edge_filter_suffix( $value, 'px' );
	}
}

if ( ! function_exists( 'barista_edge_string_starts_with' ) ) {
	/**
	 ...
	 * @return bool
	 */
	function barista_edge_string_starts_with( $haystack, $needle ) {
		if ( $haystack !== '' && $needle !== '' ) {
...
if ( ! function_exists( 'barista_edge_string_ends_with' ) ) {
	/**
	 ...
	 * @return bool
	 */
	function barista_edge_string_ends_with( $haystack, $needle ) {
		if ( $haystack !== '' && $needle !== '' ) {
...
if ( ! function_exists( 'barista_edge_dynamic_css' ) ) {
	/**
	 ...
	 *      header { width: 100%; }
	 * function call should look like this: barista_edge_dynamic_css('header', array('width' => '100%'));
	 *
	 * @return string
	 */
	function barista_edge_dynamic_css( $selector, $rules ) {
		$output = '';
...
if ( ! function_exists( 'barista_edge_get_formatted_font_family' ) ) {
	/**
	 ...
	 * @return mixed
	 */
	function barista_edge_get_formatted_font_family( $value ) {
		return str_replace( '+', ' ', $value );
	}
}

if ( ! function_exists( 'barista_edge_active_widget' ) ) {
	/**
	 * Whether widget is displayed on the front-end.
	 */
	function barista_edge_active_widget( $callback = false, $widget_id = false, $id_base = false, $skip_inactive = true ) {
...
if ( ! function_exists( 'barista_edge_execute_shortcode' ) ) {
	/**
	 ...
	 * @return mixed|string
	 */
	function barista_edge_execute_shortcode( $shortcode_tag, $atts, $content = null ) {
...
if ( ! function_exists( 'barista_edge_paged' ) ) {
	/**
	 * Function which return number of pages depends on query
	 *
	 * @return int
	 */
	function barista_edge_paged() {
...
if ( ! function_exists( 'barista_edge_is_smoth_scroll_enabled' ) ) {
	/**
	 * Function that check is smoth scroll is enabled
	 */
	function barista_edge_is_smoth_scroll_enabled() {
		$mac_os     = strpos( getenv( "HTTP_USER_AGENT" ), 'Mac' );
		$is_enabled = false;

		//is smooth scroll enabled enabled and not Mac device?
		if ( barista_edge_options()->getOptionValue( 'smooth_scroll' ) == 'yes' && $mac_os == false ) {
			$is_enabled = true;
		}
...
if ( ! function_exists( 'barista_edge_show_comments' ) ) {
	/**
	 * Functions which check are comments enabled on page
	 *
	 * @return boolean
	 */
	function barista_edge_show_comments() {
		...
		if ( get_post_meta( $id, 'edgtf_page_comments_meta', true ) == 'yes' ) {
			...
			if ( is_page() && barista_edge_options()->getOptionValue( 'page_show_comments' ) == 'yes' ) {
				$comments = true;
			} elseif ( is_singular( 'post' ) && barista_edge_options()->getOptionValue( 'blog_single_comments' ) == 'yes' ) {
				$comments = true;
			} elseif ( is_singular( 'portfolio-item' ) && barista_edge_options()->getOptionValue( 'portfolio_single_comments' ) == 'yes' ) {
				$comments = true;
			}
		}
...
if ( ! function_exists( 'barista_edge_page_comments' ) ) {
	/**
	 * Functions which show comments on page
	 *
	 * @return boolean
	 */
	function barista_edge_page_comments() {

		$comments = barista_edge_show_comments();
	...
	add_action( 'barista_edge_page_after_content', 'barista_edge_page_comments' );
}

if ( ! function_exists( 'barista_edge_page_show_pagination' ) ) {
	/**
	 * Functions which show pagination on pages
	 *
	 * @return boolean
	 */
	function barista_edge_page_show_pagination() {
...
if ( ! function_exists( 'barista_edge_icon_collections' ) ) {
	/**
	 * Returns instance of BaristaEdgeIconCollections class
	 *
	 * @return BaristaEdgeIconCollections
	 */
	function barista_edge_icon_collections() {
		return BaristaEdgeIconCollections::get_instance();
	}
}

if ( ! function_exists( 'barista_edge_get_meta_field_intersect' ) ) {
	/**
	 * @param $name
	 * @param $post_id
	 *
	 ...
	 */
	function barista_edge_get_meta_field_intersect( $name, $post_id = '' ) {
		$post_id = ! empty( $post_id ) ? $post_id : get_the_ID();

		$value = barista_edge_options()->getOptionValue( $name );
		...
		$value = apply_filters( 'barista_edge_meta_field_intersect_' . $name, $value );

		return $value;
	}
}

if ( ! function_exists( 'barista_edge_paged_home' ) ) {
	/**
	 * Set false to on paged url on homepage
	 *
	 * @return int
	 */
	function barista_edge_paged_home() {
		global $wp_query;
	...
	add_filter( 'redirect_canonical', 'barista_edge_paged_home' );
}

if ( ! function_exists( 'barista_edge_register_button' ) ) {
	/**
	 ...
	 * @return mixed
	 */
	function barista_edge_register_button( $buttons ) {
...
if ( ! function_exists( 'barista_edge_add_plugin' ) ) {

	function barista_edge_add_plugin( $plugin_array ) {
...
if ( ! function_exists( 'barista_edge_shortcodes_button' ) ) {
	/**
	 * Register Edge Shortcodes in WP Editor
	 */
	function barista_edge_shortcodes_button() {
		if ( ! current_user_can( 'edit_posts' ) && ! current_user_can( 'edit_pages' ) ) {
			return;
		}

		if ( get_user_option( 'rich_editing' ) == 'true' ) {
			add_filter( 'mce_external_plugins', 'barista_edge_add_plugin' );
			add_filter( 'mce_buttons', 'barista_edge_register_button' );
		}
	}
}

add_action( 'after_setup_theme', 'barista_edge_shortcodes_button' );
	...
	function barista_edge_add_attachment_custom_field( $form_fields, $post = null ) {
		if ( wp_attachment_is_image( $post->ID ) ) {
			...
			$form_fields['attachment_image_custom_link'] = array(
				'label'       => esc_html__( 'Image Link', 'baristawp' ),
				'input'       => 'text',
				'application' => 'image',
			...
			$form_fields['attachment_image_link_target'] = array(
				'label'       => esc_html__( 'Image Link Target', 'baristawp' ),
				'input'       => 'html',
