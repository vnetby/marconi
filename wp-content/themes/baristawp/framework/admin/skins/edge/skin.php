...
class EdgeSkin extends BaristaEdgeSkinAbstract {
    /**
     * Skin constructor. Hooks to barista_edge_admin_scripts_init and barista_edge_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'edge';

        //hook to
        add_action('barista_edge_admin_scripts_init', array($this, 'registerStyles'));
        add_action('barista_edge_admin_scripts_init', array($this, 'registerScripts'));

        add_action('barista_edge_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('barista_edge_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('barista_edge_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('barista_edge_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $scriptPath) {
            barista_edge_register_skin_script($scriptHandle, $scriptPath);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            barista_edge_register_skin_style($styleHandle, $stylePath);
        }
    ...
    public function renderOptions() {
        ...
        $tab    = barista_edge_get_admin_tab();
        $active_page = $barista_edge_Framework->edgtOptions->getAdminPageFromSlug($tab);
     ...
     *
*@param BaristaEdgeAdminPage $page current page to load
     *
     * @see EdgeSkinAbstract::loadTemplatePart()
     */
