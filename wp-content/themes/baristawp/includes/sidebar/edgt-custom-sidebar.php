...
if ( ! class_exists( 'BaristaEdgeSidebar' ) ) {

	class BaristaEdgeSidebar {
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'baristawp' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'barista-edge-sidebar', EDGE_ROOT . '/framework/admin/assets/js/edgtf-sidebar.js' );
			wp_enqueue_style( 'barista-edge-sidebar', EDGE_ROOT . '/framework/admin/assets/css/edgtf-sidebar.css' );
		}
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_html__( 'Enter Name of the new Widget Area', 'baristawp' ) . "' name='edgt-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_html__( 'Add Widget Area', 'baristawp' ) . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = array(
				...
				'after_title'   => '</h4>' . barista_edge_get_separator_html( array(
						'position'   => 'left',
						'class_name' => 'edgtf-sidebar-title-separator'
					) )
			);

			$args = apply_filters( 'barista_edge_custom_widget_args', $args );
