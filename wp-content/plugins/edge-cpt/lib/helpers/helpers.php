...
if ( ! function_exists( 'edge_core_init_shortcode_loader' ) ) {
	...
	add_action( 'barista_edge_shortcode_loader', 'edge_core_init_shortcode_loader' );
}
...
if ( class_exists( 'WP_Block_Type' ) && defined( 'EDGE_ROOT' ) ) {
	add_action( 'admin_head', 'barista_edge_meta_box_add' );
} else {
	add_action( 'add_meta_boxes', 'barista_edge_meta_box_add' );
}

if ( ! function_exists( 'edgt_core_create_meta_box_handler' ) ) {
	function edgt_core_create_meta_box_handler( $box, $key, $screen ) {
		add_meta_box(
			'edge-meta-box-' . $key,
			$box->title,
			'barista_edge_render_meta_box',
			$screen,
			'advanced',
			'high',
			array( 'box' => $box )
...
if ( ! function_exists( 'edge_core_get_shortcode_template_part' ) ) {
	/**
	 ...
	 * @return html
	 */
	function edge_core_get_shortcode_template_part( $template, $shortcode, $slug = '', $params = array() ) {
		// html content from template
		$html          = '';
		$template_path = EDGE_CORE_ABS_PATH . '/shortcodes/' . $shortcode;
		$temp          = $template_path . '/' . $template;
