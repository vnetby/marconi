...
class BaristaEdgeFramework {
	...
	private function __construct() {
		$this->edgtOptions   = BaristaEdgeOptions::get_instance();
		$this->edgtMetaBoxes = BaristaEdgeMetaBoxes::get_instance();
	}
	...
	public function setSkin( BaristaEdgeSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class BaristaEdgeSkinManager
 *
...
/**
 * Class BaristaEdgeSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class BaristaEdgeSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( barista_edge_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function getMenuIcon( $icon ) {
		...
		return barista_edge_get_skin_uri() . '/assets/img/admin-logo-icon.png';
	}
	...
	public function setShortcodeJSParams() { ?>
        <script>
            window.edgtSCIcon = '<?php echo barista_edge_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
...
class BaristaEdgeOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $barista_edge_options ) ) {
			return $barista_edge_options[ $key ];
...
/*
   Class: BaristaEdgeAdminPage
   A class that initializes Edge Admin Page
*/

class BaristaEdgeAdminPage implements iBaristaEdgeLayoutNode {
	...
	function __construct( $slug = "", $title_label = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new BaristaEdgeFieldFactory();
		$this->slug    = $slug;
	...
	public function renderChild( iBaristaEdgeRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: BaristaEdgeMetaBoxes
...
/*
   Class: BaristaEdgeMetaBox
   A class that initializes Edge Meta Box
*/

class BaristaEdgeMetaBox implements iBaristaEdgeLayoutNode {
	...
	public function setTitle( $label ) {
		...
		return $barista_edge_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

global $barista_edge_Framework;

$barista_edge_Framework = BaristaEdgeFramework::get_instance();
$edgtSkinManager        = new BaristaEdgeSkinManager();
$barista_edge_Framework->setSkin( $edgtSkinManager->getSkin() );
